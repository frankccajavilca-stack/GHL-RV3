    ...
    def delete_logo_file(cls, file_path):
        ...
        try:
            if file_path and default_storage.exists(file_path):
                ...
                print(f"Archivo no encontrado o ruta vacía: {file_path}")
...
class CompanyService:
    ...
    def store(data):
        ...
        try:
            ...
            company_name = CompanyBusinessService.validate_company_name(data.get('name', ''))
            company.name = company_name
    ...
    @staticmethod
    def process_logo(company, logo_file):
        ...
        try:
            ...
            file_path = LogoFileManager.save_logo_file(company.name, logo_file)
    ...
    def clear_company_logo(company):
        ...
        try:
            if company.logo:
                ...
                try:
                    LogoFileManager.delete_logo_file(company.logo)
                except Exception as file_error:
                    # Log del error pero continuar con la limpieza del campo
                    print(f"Error eliminando archivo físico del logo: {file_error}")
                    pass
            
            # Limpiar el campo logo en la base de datos
            company.logo = None
            company.save(update_fields=['logo'])
            
        except Exception as e:
            print(f"Error en clear_company_logo: {e}")
            raise ValueError(f"Error al eliminar el logo: {str(e)}")
